...
namespace NangaParbat
{
  ...
  //_________________________________________________________________________________
  XZCutLog::XZCutLog(DataHandler const& dataset, double const& betamin, double const& betamax, double const& alpha):
    Cut{dataset, betamin, betamax},
    _alpha(alpha)
  {
    EnforceCut();
  ...
  void XZCutLog::EnforceCut()
  {
    ...
    for (int i = 0; i < (int) bins.size(); i++)
      if (bins[i].Intz)
        if (bins[i].Intx)
          _mask[i] = (log(bins[i].zmin) - _alpha * log(bins[i].xmin) - _min > 0 && log(bins[i].zmin) - _alpha * log(bins[i].xmin) - _max < 0 ? true : false);
        else
          _mask[i] = (log(bins[i].zmin) - _alpha * log(bins[i].xav) - _min > 0 && log(bins[i].zmin) - _alpha * log(bins[i].xav) - _max < 0 ? true : false);
      else if (bins[i].Intx)
        _mask[i] = (log(bins[i].zav) - _alpha * log(bins[i].xmin) - _min > 0 && log(bins[i].zav) - _alpha * log(bins[i].xmin) - _max < 0 ? true : false);
      else
        _mask[i] = (log(bins[i].zav) - _alpha * log(bins[i].xav) - _min > 0 && log(bins[i].zav) - _alpha * log(bins[i].xav) - _max < 0 ? true : false);
  }
}
