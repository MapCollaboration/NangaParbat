              ...
              for (auto const& m : str["values"])
                {
                  ...
                  emit << YAML::Flow << YAML::BeginMap << YAML::Key << "label" << YAML::Value << "unc" << YAML::Key << "value" << YAML::Value << m["errors"][0]["symerror"].as<double>() << YAML::EndMap; // read statistical errors
                  if (PDFError)
                    {
                      // Now read PDF errors
                      getline(pdferr, line);
