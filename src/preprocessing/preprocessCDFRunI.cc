...
namespace NangaParbat
{
  ...
  std::string PreprocessCDFRunI(std::string const& RawDataPath, std::string const& ProcessedDataPath, bool const& PDFError)
  {
    ...
    for (auto const& tab : tables)
      {
        ...
        for (auto const& dv : exp["dependent_variables"])
          {
            // Open PDF-error file
            std::ifstream pdferr(PDFErrorFolder + "CDF_RunI.out");
            std::string line;
            getline(pdferr, line);
            getline(pdferr, line);
            ...
            std::map<std::string, std::string> labels
            {
              ...
              {"title", "CDF Run I, 66 GeV < Q < 116 GeV"},
              {"xlabelpy", "$q_T\\rm{ [GeV]}$"},
              {"ylabelpy", "$\\frac{d\\sigma}{dq_{T}}[\\rm{pb GeV}^{-1}]$"},
              {"titlepy", "CDF Run I, 66 GeV < Q < 116 GeV"}
            };
            ...
            for (auto const& v : dv["values"])
              {
                ...
                emit << YAML::Flow << YAML::BeginMap << YAML::Key << "label" << YAML::Value << "unc" << YAML::Key << "value" << YAML::Value << 0.01 * v["value"].as<double>() << YAML::EndMap;
                emit << YAML::EndSeq;
    ...
    return "  - {name: CDF_RunI,  file: CDF_RunI.yaml}\n";
  }
}
