...
This section is about the computation of the prefactors for the ``NLL`` (and the ``LL``)  convolution tables. Those prefactors are then read by ``tables/NNLL/IncludePrefactorsLO.py`` from the file ``tables/NNLL/FiducialCrossSectionsLO.yaml`` and put into the header of the ``NLL`` and ``LL`` tables.
...
This section is about the computation of the prefactors for the ``NNLL`` and the ``NLL'`` convolution tables. Those prefactors are then read by ``tables/NNLL/IncludePrefactorsNLO.py`` from the file ``tables/NNLL/FiducialCrossSectionsNLO.yaml`` and put into the header of the ``NNLL`` and the ``NLL'`` tables.
