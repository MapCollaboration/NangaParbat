...
namespace NangaParbat
{
   ...
   */
  class MAP22g4: public NangaParbat::Parameterisation
  {
  public:
    MAP22g4(): Parameterisation{"MAP22g4", 2, std::vector<double>{0.12840E+00, 0.28516E+00, 0.29755E+01, 0.17293E+00, 0.39432E+00, 2.12062E-01, 0.21012E+01, 0.93554E-01, 0.25246E+01, 0.52915E+01, 3.37975E-02, 0.28516E+00, 0.29755E+01, 0.17293E+00, 0.21012E+01, 0.93554E-01, 0.25246E+01, 0.01}} { };

    double Evaluate(double const& x, double const& b, double const& zeta, int const& ifunc) const
    {
      if (ifunc < 0 || ifunc >= this->_nfuncs)
        throw std::runtime_error("[MAP22g4::Evaluate]: function index out of range");
      ...
      const double g2B  = this->_pars[17];
      const double b2 = b * b;
      ...
      if (ifunc == 0)
        {
          ...
          const double alpha2  = this->_pars[12];
          //const double alpha3  = this->_pars[15];
          const double sigma1  = this->_pars[3];
          const double sigma2  = this->_pars[13];
	  ...
	  const double g1B     = N1B * pow(x / xhat, sigma2) * pow((1 - x) / (1 - xhat), pow(alpha2, 2));
          //const double g1C     = N1C * pow(x / xhat, sigma3) * pow((1 - x) / (1 - xhat), pow(alpha3, 2));
	        return NPevol * ( g1 * exp( - g1 * pow(b / 2, 2))
                          +  pow(lambda, 2)  * pow(g1B, 2) * ( 1 - g1B * pow(b / 2, 2)) * exp( - g1B * pow(b / 2, 2)))
                 / ( g1 +  pow(lambda, 2)  * pow(g1B, 2));
        }
      // TMD FFs
      else
        {
    ...
    std::string LatexFormula() const
    {
      ...
      formula += R"delimiter(\frac{g_1(x) \exp( - g_1(x) \frac{b_T^2}{4}) + \lambda^2 g_{1B}^2(x) ( 1 - g_{1B}(x) \frac{b_T^2}{4}) \exp( - g_{1B}(x) \frac{b_T^2}{4}) }{  g_1(x) +  \lambda^2 g_{1B}^2(x) }$$)delimiter";
      ...
      formula += R"delimiter($$g_{1,1B}(x) = N_{1,1B} \frac{x^{\sigma_{1,2}}(1-x)^{\alpha^2_{1,2}}}{\hat{x}^{\sigma_{1,2}}(1-\hat{x})^{\alpha^2_{1,2}}}$$)delimiter";
